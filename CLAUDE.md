...
```
MCP (server/main.py) → direct CLI → tapi-yandex-direct / browser Direct UI
```
...
- `server/cli/runner.py` — `DirectCliRunner` subprocess wrapper. `server/tools/*.py` — 151 tools across 43 modules, one module per service. `server/tools/helpers.py` — shared validation (`parse_ids`, `check_batch_limit`).
...
- Masters auth is separate from API OAuth: run `direct masters login` once to create the browser session used by the read-only `masters_get` / `masters_targetactions_get` tools. No Masters mutations are exposed.
...
- Tool-surface selection (`server/config.py`, default = all 151): `YANDEX_DIRECT_TOOL_PROFILE` (`full|core|analytics|campaign-editor`), `YANDEX_DIRECT_ENABLED_GROUPS`/`_DISABLED_GROUPS` (service / action / product-area / `financial`), `YANDEX_DIRECT_ENABLED_TOOLS`/`_DISABLED_TOOLS`.

## Tools

151 tools total (141 Direct API + 6 CLI helpers + 4 plugin) + the `oauth_login` prompt. **Canonical list: `server/contract.py`**; full tables in [docs/TOOLS.md](docs/TOOLS.md) — do not maintain a copy here. Each tool exposes a one-line description; call `tool_help('<name>')` for full docs (parameters, examples, constraints); `tool_help()` lists all.
