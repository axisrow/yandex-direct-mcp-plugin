"""Contract and argv tests for Campaign Wizard tools."""
...
LIFECYCLE_TOOL_PATHS = {
    "masters_launch": ("masters", "launch"),
    "masters_suspend": ("masters", "suspend"),
    "masters_resume": ("masters", "resume"),
    "masters_archive": ("masters", "archive"),
}
NESTED_TOOL_PATHS = {
...
@pytest.mark.parametrize("tool_name,path", LIFECYCLE_TOOL_PATHS.items())
def test_lifecycle_tools_forward_native_csv_in_one_runner_call(
    masters_module: ModuleType,
    tool_name: str,
    path: tuple[str, ...],
) -> None:
    runner = _runner_for(
        masters_module,
        [{"CampaignId": 101}, {"CampaignId": 202}, {"CampaignId": 303}],
    )

    result = getattr(masters_module, tool_name)(" 101, 202,303 ")
...
@pytest.mark.parametrize("tool_name", LIFECYCLE_TOOL_PATHS)
def test_lifecycle_tools_reject_blank_campaign_ids(
    masters_module: ModuleType,
    tool_name: str,
) -> None:
    runner = _runner_for(masters_module, [])

    result = getattr(masters_module, tool_name)("   ")

    assert result["error"] == "missing_campaign_ids"
    runner.run_json_lenient.assert_not_called()


def test_lifecycle_tools_enforce_batch_limit(masters_module: ModuleType) -> None:
    runner = _runner_for(masters_module, [])

    result = masters_module.masters_suspend(",".join(str(i) for i in range(11)))

    assert result["error"] == "batch_limit"
    runner.run_json_lenient.assert_not_called()


def test_lifecycle_tools_forward_browser_env(
    masters_module: ModuleType,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runner = _runner_for(masters_module, {"CampaignId": 101})
    ...
    masters_module.masters_resume("101")
...
def test_masters_copy_defaults_to_draft(masters_module: ModuleType) -> None:
    runner = _runner_for(masters_module, {"CampaignId": 202})

    result = masters_module.masters_copy("101")

    assert result == {"CampaignId": 202}
    runner.run_json_lenient.assert_called_once_with(
        ["masters", "copy", "101", "--format", "json"]
    )
...
def test_masters_signature_defaults_and_cli_option_tables(
    ...
    for tool_name in LIFECYCLE_TOOL_PATHS:
        parameter = signatures[tool_name].parameters["campaign_ids"]
        assert parameter.default is inspect.Parameter.empty
        assert parameter.annotation is str
...
@pytest.mark.parametrize("tool_name,path", STRING_ID_TOOL_PATHS.items())
def test_click_integer_ids_are_safe_mcp_strings(
    masters_module: ModuleType,
    tool_name: str,
    path: tuple[str, ...],
) -> None:
...
def test_eleven_masters_tools_fit_token_budget() -> None:
    ...
    for tool in tools:
        if tool.name in STRING_ID_TOOL_PATHS:
