---
name: yandex-direct
description: "Управление Яндекс.Директ через MCP tools. Используется когда пользователь упоминает Яндекс.Директ, хочет управлять кампаниями, объявлениями, ключевыми словами, ставками, бюджетом, или получить статистику."
user-invocable: true
...
- **Максимум 3 «комбинаторных» черновика на группу** (ТГО-кампании, TEXT_AD):
  четвёртое объявление отклоняется с `Error 7001` («Достигнуто максимальное
  количество комбинаторных объявлений в группе - 3»), причём батч из 4
  объявлений в одну группу отклоняется целиком. Режьте батчи по ≤3 объявления
  на группу. Параллельные одиночные вызовы могут обойти проверку гонкой и
  создать больше трёх — не полагайтесь на это, ограничивайте на своей стороне.
- **int64-ID объявлений**: новые «комбинаторные» объявления получают ID вида
  `1915883588174806058` (> 2^53). ID-параметры инструментов (`ads_update(id=...)`
  и др.) принимают **строку** — передавайте такие ID строкой, чтобы не потерять
  точность на границе JSON.
...
Direct возвращает этот код, чтобы сообщить, что какой-то параметр был
проигнорирован. Поле `Details` предупреждения (называет конкретный параметр)
пробрасывается в ответе инструмента as-is — смотрите его в `Warnings[].Details`.

Частый случай (поведение Яндекса 2026): для TEXT_AD `Title2` теперь склеивается
в единый `Title` (суммарный лимит 56 символов), а отдельный `Title2`
отбрасывается — `ads_add` завершается успешно, но возвращает Warning 10165.
Последующий `ads_update` `Title`/`Title2` такого объявления — тихий no-op.
Держите совокупный заголовок в пределах 56 символов. Это upstream-поведение
Яндекса, а не преобразование плагина.

Чтобы увидеть исходящий payload до отправки, у мутирующих tools есть `dry_run=True`:
